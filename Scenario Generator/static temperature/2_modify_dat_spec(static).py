...
for scenario_num in range(total_scenario):
    ...
    util.modify_aero_emit_dist(directory, matrix, ss_option=True, dust_option=True)
    
    util.modify_gas_emit(directory, matrix, DMS_option=True)
    
    util.temp_profile_generator(directory, matrix)
