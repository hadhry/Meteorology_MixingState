...
max_model = pickle.load(open('predictionModels/model1Upper.pkl', 'rb'))
min_model = pickle.load(open('predictionModels/model1Lower.pkl', 'rb'))


def temp_emulator(long, lat, doy):
    input = np.array([[long, lat, doy]])
    max_temp = max_model.predict(input)[0]
    min_temp = min_model.predict(input)[0]
...
## Environmental variable                                               Need to add / replace for longitude
# 1) RH   
...
# 5) Longitude
Longitude_min = -179.99; Longitude_max = 179.99; #39
# Dilution rate
# To be added
# Mixing height
# To be added
...
for i in range(lhs.shape[0]):
    ...
    min_temp, max_temp = temp_emulator(long, lat, doy)
...
# Load the matrix and distribute into different scenarios               Just modify the files so that they can be copied fixed
