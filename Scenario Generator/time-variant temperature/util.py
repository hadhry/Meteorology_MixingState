...
profileModel = pickle.load(open('predictionModels/model2.pkl', 'rb'))
# %% Subfunction
...
def modify_gas_emit_func(flist_row, coefficient):
    ...
    flist_split_np=np.array(flist_split_np * coefficient,dtype=str)
    #print(flist_split_np)
...
def modify_temp_func(flist_row, coefficient):
    ...
    # multiply by coefficient
    flist_split_np=np.array(np.repeat(coefficient,len(flist_split_np)),dtype=str)
    #print(flist_split_np)
...
def temp_profile_generator(directory, matrix):
    long = matrix[39]
    lat = matrix[1]
    doy = matrix[2]
    temp = matrix[3]

    input = np.array([[long, lat, doy, temp]])
    profile = profileModel.predict(input)[0]

    print(temp.shape)
    print(profile.shape)
    profile = np.concatenate([temp.reshape((1,)),profile])

    # Create the time array using the timestep information (assuming one hour timestep)
    time_seconds = np.arange(0, len(profile) * 3600, 3600)

    # Convert time and temperature arrays to strings
    time_str = "\t".join([str(t) for t in time_seconds])
    temp_str = "\t".join([f"{temp:.3f}" for temp in profile])

    combined_string = f"#\ttime\t(s)\n#\ttemp\t(K)\ntime\t{time_str}\ntemp\t{temp_str}"

    with open(directory+"/temp.dat", "w") as file:
...
def make_spec(directory, scenario_num, matrix):
    ...
    flist[29] = "rel_humidity " + "{:.4}".format(matrix[0]) + "               # initial relative humidity (1) \n"
    print("Relative Humidity", "{:.4}".format(matrix[0]))
   
    flist[30] = "latitude  " + "{:.4}".format(matrix[1]) + "                      # latitude (degrees, -90 to 90) \n"
    print("latitude  " + "{:.4}".format(matrix[1]))

    flist[31] = "longitude " + "{:.4}".format(matrix[39]) + "                     # longitude (degrees, -180 to 180) \n"   
    print("longitude  " + "{:.4}".format(matrix[39]))
   
    flist[34] = "start_day " + str(int(matrix[2])) + "                   # start day of year (UTC) \n"
...
def make_spec_restart(directory, scenario_num, matrix):
    ...
    # modify the matrix here
    # flist[1] = "output_prefix out/urban_plume_" + str(scenario_num).zfill(4) + "   # prefix of output files \n"
    restart_time_stamp = "%08i" % int(25)
    
    flist[7] = "restart_file out_init/urban_plume_0001_" + restart_time_stamp + ".nc \n"
    print("restart_file out_init/urban_plume_0001_" + restart_time_stamp + ".nc \n")
   
    flist[23] = "rel_humidity " + "{:.4}".format(matrix[0]) + "               # initial relative humidity (1) \n"
    print("Relative Humidity", "{:.4}".format(matrix[0]))
   
    flist[24] = "latitude  " + "{:.4}".format(matrix[1]) + "                      # latitude (degrees, -90 to 90) \n"
    print("latitude  " + "{:.4}".format(matrix[1]))

    flist[25] = "longitude " + "{:.4}".format(matrix[39]) + "                     # longitude (degrees, -180 to 180) \n"   
    print("longitude  " + "{:.4}".format(matrix[39])) 
   
    flist[28] = "start_day " + str(int(matrix[2])) + "                   # start day of year (UTC) \n"
    print("start_day " + str(int(matrix[2])))    
